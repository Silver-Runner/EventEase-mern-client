    ...
    <div className="lg:bg-gray-200 h-full p-5 w-full ">
      <div className="flex flex-col gap-1 mt-5">
        <h1 className="text-2xl font-bold text-info">
          Event <b className="text-primary font-bold pl-2 ">Ease</b>
        </h1>
      ...
      <div className="flex flex-col gap-10 mt-20">
        {menuToRender.map((item: any) => (
          <div  key={item.name}
          className={`cursor-pointer px-5 py-3 rounded flex gap-5 text-sm items-center ${item.isActive ? 'bg-info text-white':''}` } onClick={() => {
            if(item.name === "Logout"){
              onLogout();
            }else{
              navigate(item.path);
